...
class Translator:
    def __init__(self, target_lang):
        self.keys = {}
        if target_lang == "en":
            target_lang = None  # default = None = "en"
        self.target_lang = target_lang
        self.records = set()

    def load_translation_file(self, path):
        if extract_language_from_path(path) == self.target_lang:
            translation_file = open(path)
...
def build_event_type_property_line(prop, translator):
    name = prop["name"]
    normalized_name = name.replace(".", "_")
    return [name, prop["type"], translator.get_or_default(f"{normalized_name}_definition", prop["definition"])]
...
def event_type_properties_list(event_type, translator, file):
    ...
    for prop in event_type["properties"]:
        ...
        table.append_line(build_event_type_property_line(prop, translator))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--translation", type=str, action="append", help="translation file path")
    ...
    if args.translation:
        for path in args.translation:
            translator.load_translation_file(path)
